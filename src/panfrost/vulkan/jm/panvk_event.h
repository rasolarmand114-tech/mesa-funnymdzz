 ...
 * (KBASE_JM_SOFT_EVENT_SET / KBASE_JM_SOFT_EVENT_RESET) living inside a
 * tiny priv BO. Updates go through kbase_jm_soft_event_update() so the
 * kernel's own soft-event state stays authoritative -- we never just poke
 * the mapping from the host -- which is what makes this "GPU-visible"
 * rather than a plain CPU-only flag.
 */
struct panvk_event {
...
/* Push a new status (KBASE_JM_SOFT_EVENT_SET or KBASE_JM_SOFT_EVENT_RESET)
 * to the event's soft-event byte via kbase_jm_soft_event_update(). Returns
 * false on failure, in which case the caller should treat the device as
 * lost. */
